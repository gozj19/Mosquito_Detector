...
#define SAMPLES 512                // FFT 샘플 개수 (2의 제곱수 권장)
...
void loop() {
  ...
  // --- 3. 감지된 마이크 순서대로 회전 및 스프레이 분사 ---
  if (detectionCount > 0) {
    // peakValue 기준으로 내림차순 정렬
    for (int i = 0; i < detectionCount - 1; i++) {
      for (int j = i + 1; j < detectionCount; j++) {
        if (detections[i].peakValue < detections[j].peakValue) {
    ...
    for (int i = 0; i < detectionCount; i++) {
      rotateToMic(detections[i].micIndex);
      spray();
    }
  }
